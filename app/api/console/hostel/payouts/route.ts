...
import { getHostelPayoutAccount, hostelPayoutStatement, listHostelPayoutLandlords, platformHostelPayoutBalance, recordHostelPayoutBatch } from "@/lib/hostel-engine/payouts";
...
export async function GET(request: Request) {
  try {
    ...
    if (!landlordId) {
      const [{ landlords, totals }, balance] = await Promise.all([listHostelPayoutLandlords(), platformHostelPayoutBalance()]);
      return Response.json({ ok: true, landlords, totals, balance }, { headers: NO_STORE });
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (!limited.ok) return rateLimitResponse(limited.retryAfter);
    const body = await request.json() as { landlordId?: unknown; reference?: unknown; note?: unknown };
