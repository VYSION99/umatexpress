...
/**
 * A moderator acts on one room: end it, or remove one message.
 *
 * Both actions are audited against the staff account, and both are conditional
 * in the engine — ending an already-ended room and removing an already-removed
 * message report the truth rather than succeeding twice.
 */
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const body = await request.json() as { action?: unknown; messageId?: unknown };
